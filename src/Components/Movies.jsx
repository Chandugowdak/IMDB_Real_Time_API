...
const Movies = ({ handlingmovie, MoviRemover, watechlater }) => {
  ...
  useEffect(() => {
    axios.get(`https://api.themoviedb.org/3/movie/popular?api_key=b1edfc35cc0588e7843db093ef9c6953&language=en-US&page=${pagenumber}`)
      .then(function (res) {
      ...
      <div className='card'>
        <div className='row'>
          {Movie.map((MovieObject) => {
            return (
              <div key={MovieObject.id} className='col-12 col-sm-6 col-md-4 col-lg-3 mb-3'>
                ...
                <Moviecard
                  ...
                  handlingmovie={handlingmovie}
                  MovieObject={MovieObject}
                  MoviRemover={MoviRemover}
                  watechlater={watechlater}
                />
              </div>
            );
          })}
